...
function App() {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${t.font} ${isRtl ? 'rtl' : 'ltr'} bg-brand-softbg selection:bg-brand-green selection:text-white`}>
      ...
      {/* ------------------------------------------------------------- */}
      {/* PITCH - PROPOSITION DE COLLABORATION SECTION (At the very bottom) */}
      {/* ------------------------------------------------------------- */}
      <section className="bg-slate-50 border-t-8 border-brand-green py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          
          {/* Header */}
          <div className="text-center mb-12">
            <span className="inline-block bg-brand-lightgreen border border-brand-green/20 px-3 py-1 rounded-full text-brand-green font-extrabold text-xs tracking-wider uppercase mb-2">
              Developer Zone
            </span>
            <h2 className="text-3xl font-extrabold text-brand-teal tracking-wide">
              {t.pitch.title}
            </h2>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-stretch">
            ...
            {/* Why this site? */}
            <div className="lg:col-span-4 bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col justify-between">
              <div>
                <h3 className="text-xl font-extrabold text-brand-teal mb-6 pb-2 border-b border-gray-100 flex items-center gap-2">
                  <span className="w-2.5 h-6 bg-brand-green rounded-full"></span>
                  {t.pitch.whyTitle}
                </h3>
                <ul className="space-y-4">
                  {t.pitch.whyItems.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <div className="bg-brand-lightgreen text-brand-green p-1 rounded-full mt-0.5 flex-shrink-0">
            ...
            <div className="lg:col-span-4 bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col justify-between">
              <div>
                ...
                <ul className="space-y-4">
                  {t.pitch.offerItems.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <div className="bg-brand-lightgreen text-brand-green p-1 rounded-full mt-0.5 flex-shrink-0">
                        <Check className="w-4 h-4" />
                      </div>
            ...
            {/* Investment Box */}
            <div className="lg:col-span-4 bg-white p-8 rounded-3xl shadow-lg border-2 border-brand-green flex flex-col justify-between relative overflow-hidden">
              {/* Highlight badge */}
              <div className="absolute top-0 right-0 bg-brand-green text-white text-[10px] font-extrabold px-3 py-1 rounded-bl-xl uppercase tracking-wider">
                Premium
              </div>
              
              <div>
                <h3 className="text-lg font-bold text-gray-400 uppercase tracking-widest mb-6">
                  {t.pitch.investmentTitle}
                </h3>
                <p className="text-5xl font-black text-brand-teal tracking-tight mb-2">
                  {t.pitch.investmentValue}
                </p>
                <div className="w-full h-px bg-gray-100 my-6"></div>
                
                <h4 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-2">
                  {t.pitch.deliveryTitle}
                </h4>
                <p className="text-lg font-bold text-brand-teal flex items-center gap-2">
                  <Clock className="w-5 h-5 text-brand-green" />
                  <span>{t.pitch.deliveryValue}</span>
                </p>
              </div>

              <div className="mt-8">
                <button className="w-full bg-brand-green hover:bg-brand-darkgreen text-white font-extrabold py-4 px-6 rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 group">
                  <span>{t.pitch.btnDemo}</span>
          ...
          {/* Bottom Freelance Banner Bar */}
          <div className="mt-12 bg-brand-teal text-white p-6 rounded-3xl shadow-lg flex flex-col lg:flex-row items-center justify-between gap-6">
            <div className="flex flex-col lg:flex-row items-center gap-4 text-center lg:text-left rtl:lg:text-right">
              <div className="bg-white/10 p-3 rounded-2xl text-emerald-300">
                <Award className="w-8 h-8" />
              </div>
              <div>
                <h4 className="text-lg font-extrabold flex flex-wrap items-center justify-center lg:justify-start gap-2">
                  <span>{t.pitch.developerTitle}</span>
                  <span className="bg-emerald-500/20 text-emerald-300 text-xs px-2.5 py-0.5 rounded-full font-bold">Available</span>
                </h4>
                <p className="text-sm text-emerald-100 opacity-90 mt-1">{t.pitch.developerSub}</p>
              </div>
            </div>
            
            <div className="flex flex-col items-center lg:items-end gap-3">
              <div className="flex flex-wrap justify-center gap-2.5">
                {t.pitch.qualities.map((qual, idx) => (
                  <span key={idx} className="bg-white/15 px-3 py-1 rounded-full text-xs font-bold text-emerald-200">
                    {qual}
                  </span>
                ))}
              </div>
